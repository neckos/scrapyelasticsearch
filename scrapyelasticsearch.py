#Source https://github.com/jayzeng/scrapy-elasticsearch
#Updated: kaspars.zemiitis@gmail.com, added some itemDrop check.
...
class ElasticSearchPipeline(object):
    ...
    def process_item(self, item, spider):
        if isinstance(item, types.GeneratorType) or isinstance(item, list):
            ...
            #customized to drop field with empty field (defined in settings)
            if self.settings['ELASTICSEARCH_SAVE_IF_FIELD_IS_NOT_EMPTY'] is not None:
                if item.get(self.settings['ELASTICSEARCH_SAVE_IF_FIELD_IS_NOT_EMPTY']) and item[self.settings['ELASTICSEARCH_SAVE_IF_FIELD_IS_NOT_EMPTY']]:
                    self.index_item(item)
                    logging.debug('Item sent to Elastic Search %s' % self.settings['ELASTICSEARCH_INDEX'])
                    return item
                else:
                    raise DropItem('Missing information on item %s' % ('scraped from ' + item.get('url') or item.get('title')))
            else:
                return item
